...
import {
  fetchUnreadMemberInboxEvents,
  markMemberInboxEventsRead,
...
export async function GET() {
  ...
  try {
    const events = await fetchUnreadMemberInboxEvents(supabase, user.id);
    return NextResponse.json({ events });
  } catch (e) {
...
export async function POST(request: Request) {
  ...
  try {
    await markMemberInboxEventsRead(supabase, user.id, ids);
    return NextResponse.json({ ok: true });
  } catch (e) {
