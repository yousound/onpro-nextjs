-- Exclude operator self-team contact from workspace email matches (false "Join team" on own workspace).
  ...
  matched_contacts AS (
    ...
    WHERE c.role IN ('Client', 'Team', 'Vendor')
      ...
      AND (
        ...
        OR (
          c.user_id IS DISTINCT FROM p_member_user_id
          AND c.id IS DISTINCT FROM (
            SELECT self_contact_id FROM profiles WHERE id = p_member_user_id
          )
        )
      )
  ),
  invite_matches AS (
    ...
    WHERE LOWER(TRIM(pi.email)) = n.email
      ...
      AND (
        p_member_user_id IS NULL
        OR (
          pi.operator_user_id IS DISTINCT FROM p_member_user_id
          AND c.id IS DISTINCT FROM (
            SELECT self_contact_id FROM profiles WHERE id = p_member_user_id
          )
        )
